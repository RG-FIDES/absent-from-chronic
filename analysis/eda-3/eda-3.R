...
# Key variable: absence_days_chronic — work days missed due to a chronic health condition
...
ds0 <- ds0 |>
  dplyr::mutate(
    absence_days_chronic = as.integer(absence_days_chronic),
    has_any_chronic      = dplyr::if_else(is.na(absence_days_chronic), NA,
                                          absence_days_chronic > 0L)
  )

# ---- inspect-data-0 ----------------------------------------------------------
cat("Data Overview:\n")
cat("  - ds0 (cchs_employed):", format(nrow(ds0), big.mark = ","), "rows x", ncol(ds0), "cols\n")
cat("  - NA absence_days_chronic:", sum(is.na(ds0$absence_days_chronic)), "\n")
cat("  - 0 days chronic absent:  ", sum(ds0$absence_days_chronic == 0L, na.rm = TRUE), "\n")
cat("  - 1+ days chronic absent: ", sum(ds0$absence_days_chronic >= 1L, na.rm = TRUE), "\n")
...
ds0 |>
  dplyr::select(absence_days_chronic, sex_label, age_group_3, survey_cycle_label) |>
  summary() |>
  print()

# ---- tweak-data-1 ------------------------------------------------------------
# ds1: analysis-ready subset — respondents with 1+ chronic absence days reported.
# Excludes: NAs (chronic absence not reported) and 0-day respondents (no chronic absence event).
# ds1 is the working dataset for all g1-family graphs and downstream analysis.

n_excluded_na   <- sum(is.na(ds0$absence_days_chronic))
n_excluded_zero <- sum(ds0$absence_days_chronic == 0L, na.rm = TRUE)

ds1 <- ds0 |>
  dplyr::filter(!is.na(absence_days_chronic), absence_days_chronic > 0L)

message("ds1 created: ", format(nrow(ds1), big.mark = ","), " respondents with 1+ chronic absence days")
...
# ---- inspect-data-3 ----------------------------------------------------------
cat("\nds1 Overview (1+ chronic absence days only):\n")
cat("  - Rows:   ", format(nrow(ds1), big.mark = ","), "\n")
cat("  - Median absence_days_chronic:", median(ds1$absence_days_chronic), "\n")
cat("  - Mean   absence_days_chronic:", round(mean(ds1$absence_days_chronic), 1), "\n")
cat("  - Min / Max:                  ", min(ds1$absence_days_chronic),
    "/", max(ds1$absence_days_chronic), "\n")

# ---- g1-data-prep ------------------------------------------------------------
# Aggregate ds1: one row per unique absence_days_chronic value with respondent count.
...
g1_data   <- ds1 |>
  dplyr::count(absence_days_chronic, name = "n_people")

g1_median <- median(ds1$absence_days_chronic)
g1_mean   <- round(mean(ds1$absence_days_chronic), 1)

message("g1_data prepared: ", nrow(g1_data), " unique day values")
message("  Median: ", g1_median, " days | Mean: ", g1_mean, " days")

# ---- analytic-chronic-ratio --------------------------------------------------
# Compute the share of employed respondents reporting zero chronic absence days vs. 1+.
# Purpose: context for the reader — how common is any chronic absence at all?
...
chronic_ratio_tbl <- dplyr::tibble(
  group              = c("No chronic absence (0 days)", "Any chronic absence (1+ days)", "Not reported (NA)"),
  n                  = c(n_excluded_zero, nrow(ds1), n_excluded_na),
  pct_of_respondents = c(pct_zero, pct_one_plus, NA_real_)
)

message("Chronic absence ratio among employed Canadians who answered:")
print(chronic_ratio_tbl)
message(pct_zero, "% reported no chronic absence days vs. ",
        pct_one_plus, "% with 1+ days.")

# ---- g1-scatter --------------------------------------------------------------
# Scatter: x = absence_days_chronic (discrete day count), y = number of people.
...
n_over_40 <- sum(ds1$absence_days_chronic > 40L)

g1_scatter <- g1_data |>
  ggplot(aes(x = absence_days_chronic, y = n_people)) +
  ...
  labs(
    title    = "Chronic Absence Days vs. Number of Respondents (1+ days, zoomed to 1-40)",
    ...
    x        = "Total chronic absence days reported",
...
# ---- g1-hist -----------------------------------------------------------------
# Histogram of absence_days_chronic from ds1 (1+ days, no NAs, no 0s).
...
g1_hist <- ds1 |>
  ggplot(aes(x = absence_days_chronic)) +
  ...
  coord_cartesian(xlim = c(1, 40)) +
  labs(
    title    = "Distribution of Chronic Absence Days — 1+ Days Only (zoomed to 1-40)",
...
# g2 family explores whether the chronic absence-day distribution differs by sex.
# ds1 is the source — same filtered population as g1.

g2_stats <- ds1 |>
  dplyr::group_by(sex_label) |>
  dplyr::summarise(
    median_days = median(absence_days_chronic),
    mean_days   = round(mean(absence_days_chronic), 1),
    n_people    = dplyr::n(),
    .groups     = "drop"
  )

n_over_40_g2 <- sum(ds1$absence_days_chronic > 40L)

message("g2_stats (per sex):")
print(g2_stats)

# ---- analytic-sex-ratio ------------------------------------------------------
# Ratio of 0-chronic-absence vs. 1+ chronic absence respondents broken down by sex.
# Uses ds0 (all who answered) — not ds1 — so the 0-day group is retained.

sex_ratio_tbl <- ds0 |>
  dplyr::filter(!is.na(absence_days_chronic)) |>
  dplyr::mutate(
    absence_group = dplyr::if_else(absence_days_chronic == 0L,
                                   "No chronic absence (0 days)", "Any chronic absence (1+ days)")
  ) |>
...
message("Sex-specific chronic absence ratio (0-day vs. 1+ days):")
print(sex_ratio_tbl)
...
g2_hist_sex <- ds1 |>
  ggplot(aes(x = absence_days_chronic, fill = sex_label)) +
  ...
  labs(
    title    = "Chronic Absence Day Distribution by Sex (1+ days, zoomed to 1-40)",
...
# g3 family explores whether chronic absence-day distribution differs by age group.
...
n_over_40_g3 <- sum(ds1$absence_days_chronic > 40L)
...
g3_hist_age <- ds1 |>
  ggplot(aes(x = absence_days_chronic, fill = age_group_3)) +
  geom_histogram(binwidth = 5, colour = "white", alpha = 0.85) +
  ...
  labs(
    title    = "Chronic Absence Day Distribution by Age Group (1+ days, zoomed to 1-40)",
    subtitle = paste0("n = ", format(nrow(ds1), big.mark = ","),
                      " respondents | 5-day bins",
...
# Purpose: check whether the chronic absence-day distribution differs across CCHS cycles.

g4_data <- ds1 |>
  dplyr::count(survey_cycle_label, absence_days_chronic, name = "n_people")
...
n_over_40_g4 <- sum(ds1$absence_days_chronic > 40L)
...
g4_scatter_cycle <- g4_data |>
  ggplot(aes(x = absence_days_chronic, y = n_people, colour = survey_cycle_label)) +
  ...
  labs(
    title    = "Chronic Absence Days vs. Respondents by Survey Cycle (1+ days, zoomed to 1-40)",
...
# ---- g4-hist-cycle -----------------------------------------------------------
g4_hist_cycle <- ds1 |>
  ggplot(aes(x = absence_days_chronic, fill = survey_cycle_label)) +
  geom_histogram(binwidth = 5, colour = "white", alpha = 0.85) +
  geom_vline(data = g4_stats,
             aes(xintercept = median_days),
             colour = "firebrick", linetype = "dashed", linewidth = 0.8) +
  ...
  facet_wrap(~ survey_cycle_label, ncol = 2) +
  labs(
    title    = "Chronic Absence Day Distribution by Survey Cycle (1+ days, zoomed to 1-40)",
    subtitle = paste0("n = ", format(nrow(ds1), big.mark = ","),
                      " respondents | 5-day bins",
                      ifelse(n_over_40_g4 > 0,
                             paste0(" (", n_over_40_g4, " with >40 days outside view)"),
...
# ---- g5-data-prep ------------------------------------------------------------
# g5 family: chronic absence-day distribution split by education_level.
...
n_over_40_g5 <- sum(ds5$absence_days_chronic > 40L)
...
# ---- g5-hist-edu -------------------------------------------------------------
g5_hist_edu <- ds5 |>
  ggplot(aes(x = absence_days_chronic, fill = education_level)) +
  geom_histogram(binwidth = 5, colour = "white", alpha = 0.85) +
  ...
  labs(
    title    = "Chronic Absence Day Distribution by Education Level (1+ days, zoomed to 1-40)",
    subtitle = paste0("n = ", format(nrow(ds5), big.mark = ","),
                      " respondents | 5-day bins",
                      ifelse(n_over_40_g5 > 0,
                             paste0(" (", n_over_40_g5, " with >40 days outside view)"),
                             ""),
                      " | 0-day, NA chronic absence, and NA education excluded"),
    x        = "Total chronic absence days reported",
    y        = "Number of respondents",
    caption  = "Source: Statistics Canada, CCHS 2010-2011 & 2013-2014"
  ) +
...
# ---- g6-data-prep ------------------------------------------------------------
# g6 family: chronic absence-day distribution split by marital_status_label.
...
n_over_40_g6 <- sum(ds6$absence_days_chronic > 40L)
...
# ---- g6-hist-marital ---------------------------------------------------------
g6_hist_marital <- ds6 |>
  ggplot(aes(x = absence_days_chronic, fill = marital_status_label)) +
  geom_histogram(binwidth = 5, colour = "white", alpha = 0.85) +
  ...
  labs(
    title    = "Chronic Absence Day Distribution by Marital Status (1+ days, zoomed to 1-40)",
    subtitle = paste0("n = ", format(nrow(ds6), big.mark = ","),
                      " respondents | 5-day bins",
                      ifelse(n_over_40_g6 > 0,
                             paste0(" (", n_over_40_g6, " with >40 days outside view)"),
                             ""),
                      " | 0-day, NA chronic absence, and NA marital status excluded"),
    x        = "Total chronic absence days reported",
    y        = "Number of respondents",
    caption  = "Source: Statistics Canada, CCHS 2010-2011 & 2013-2014"
  ) +
...
# ---- g7-data-prep ------------------------------------------------------------
# g7 family: chronic absence-day distribution split by immigration_status_label.
...
n_over_40_g7 <- sum(ds7$absence_days_chronic > 40L)
...
g7_hist_immigration <- ds7 |>
  ggplot(aes(x = absence_days_chronic, fill = immigration_status_label)) +
  geom_histogram(binwidth = 5, colour = "white", alpha = 0.85) +
  ...
  labs(
    title    = "Chronic Absence Day Distribution by Immigration Status (1+ days, zoomed to 1-40)",
    subtitle = paste0("n = ", format(nrow(ds7), big.mark = ","),
                      " respondents | 5-day bins",
                      ifelse(n_over_40_g7 > 0,
                             paste0(" (", n_over_40_g7, " with >40 days outside view)"),
                             ""),
                      " | 0-day, NA chronic absence, and NA immigration status excluded"),
    x        = "Total chronic absence days reported",
    y        = "Number of respondents",
    caption  = "Source: Statistics Canada, CCHS 2010-2011 & 2013-2014"
  ) +
